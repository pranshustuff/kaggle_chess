...
void Position::move_do(const Move move){
    switch(move_type(move)){
        ...
        case NullMove:      { move_null_do(); break; }
        default:{
            std::cerr<<"Wrong move type ("<<move_type(move)<<")!"<<std::endl;
            exit(1);
        }
    };
...
void Position::move_undo(){
    ...
    switch(type){
        ...
        case NullMove:        { break; }
        default:{
            std::cerr<<"Wrong move type (undo)!"<<std::endl;
            exit(1);
        }
    };
...
void Position::move_quiet_do(const Move move){
    ...
    current->pvt[Opening] = ph[ply-1].pvt[Opening] + PVT[p][to][Opening]
                            - PVT[p][from][Opening];
    current->pvt[Endgame] = ph[ply-1].pvt[Endgame] + PVT[p][to][Endgame]
                            - PVT[p][from][Endgame];
    current->fifty = ph[ply-1].fifty + 1;
    current->castle = ph[ply-1].castle;
    current->capture = NO_PIECE;
    current->ep = NO_SQ;
        const Bitboard bb = Bitboard(1) << from | Bitboard(1) << to;
    bitboard[stm] ^= bb;
    bitboard[p] ^= bb;
    occupied ^= bb;
    piece[from] = NO_PIECE;
...
void Position::move_capture_do(const Move move){
    ...
    current->pvt[Opening] = ph[ply-1].pvt[Opening] + PVT[p][to][Opening]
                            - PVT[p][from][Opening] - PVT[c][to][Opening];
    current->pvt[Endgame] = ph[ply-1].pvt[Endgame] + PVT[p][to][Endgame]
                            - PVT[p][from][Endgame] - PVT[c][to][Endgame];
    current->fifty = 0;
    current->castle = ph[ply-1].castle;
    current->capture = c;
    current->ep = NO_SQ;
    ...
    const Bitboard bbTo = Bitboard(1) << to;
    const Bitboard bbMove = Bitboard(1) <<from | bbTo;
    occupied ^= Bitboard(1) << from;
    bitboard[stm] ^= bbMove;
    bitboard[p] ^= bbMove;
    bitboard[stm^1] ^= bbTo;
    bitboard[c] ^= bbTo;
...
void Position::move_ep_do(const Move move){
    ...
    current->mat_key = ph[ply-1].mat_key ^ MatKey[c][num[c]];
    current->pvt[Opening] = ph[ply-1].pvt[Opening] + PVT[p][to][Opening]
                            - PVT[p][from][Opening] - PVT[c][cSq][Opening];
    current->pvt[Endgame] = ph[ply-1].pvt[Endgame] + PVT[p][to][Endgame]
                            - PVT[p][from][Endgame] - PVT[c][cSq][Endgame];
    current->fifty = 0;
    current->castle = ph[ply-1].castle;
    current->capture = c;
    current->ep = NO_SQ;
    ...
    num[stm^1] --;
    num[c] --;
        const Bitboard bbMove = Bitboard(1) <<from | Bitboard(1) <<to;
        const Bitboard bbCapture = Bitboard(1) << cSq;
    bitboard[stm] ^= bbMove;
    bitboard[p] ^= bbMove;
...
void Position::move_castle_do(const Move move){
    ...
    ply++;
    current= &ph[ply];
    current->key = ph[ply-1].key ^ SqKey[p][from] ^ SqKey[p][to]
                   ^ SqKey[r][rFrom] ^ SqKey[r][rTo];
    current->pawn_key = ph[ply-1].pawn_key;
    current->mat_key = ph[ply-1].mat_key;
    current->pvt[Opening] = ph[ply-1].pvt[Opening] + PVT[p][to][Opening]
                            + PVT[r][rTo][Opening] - PVT[p][from][Opening]
                            - PVT[r][rFrom][Opening];
    current->pvt[Endgame] = ph[ply-1].pvt[Endgame] + PVT[p][to][Endgame]
                            + PVT[r][rTo][Endgame] - PVT[p][from][Endgame]
                            - PVT[r][rFrom][Endgame];
    current->fifty = 0;
    current->capture = NO_PIECE;
    current->ep = NO_SQ;
    ...
    if(stm == White){
        ...
        const Bitboard bbMove = Bitboard(1)<<from | Bitboard(1)<<to;
        const Bitboard bbRook = Bitboard(1)<<rFrom | Bitboard(1)<<rTo;
...
void Position::move_promotion_do(const Move move){
    ...
    current->mat_key = ph[ply-1].mat_key ^ MatKey[p][num[p]]
                       ^ MatKey[pp][num[pp]+1];
    current->pvt[Opening] = ph[ply-1].pvt[Opening] + PVT[pp][to][Opening]
                            - PVT[p][from][Opening];
    current->pvt[Endgame] = ph[ply-1].pvt[Endgame] + PVT[pp][to][Endgame]
                            - PVT[p][from][Endgame];
    current->fifty = 0;
    current->castle = ph[ply-1].castle;
    current->capture = NO_PIECE;
    ...
    num[pp] ++;
        ...
        const Bitboard bbFrom = Bitboard(1)<<from;
        const Bitboard bbTo = Bitboard(1)<<to;
...
int Position::see(const Move move) const{
    ...
    attack = bb_rook_attack(to,bb)
             & (bitboard[WR] | bitboard[BR] | bitboard[WQ] | bitboard[BQ]);
    attack |= bb_bishop_attack(to,bb)
              & (bitboard[WB] | bitboard[BB] | bitboard[WQ] | bitboard[BQ]);
    attack |= knight_attack(to) & (bitboard[WN] | bitboard[BN]);
    attack |= king_attack(to) & (bitboard[WK] | bitboard[BK]);
    attack |= (shift_up_right(Bitboard(1)<<to) | shift_up_left(Bitboard(1)<<to)) & bitboard[BP];
    attack |= (shift_down_right(Bitboard(1)<<to) | shift_down_left(Bitboard(1)<<to)) & bitboard[WP];
    attack &= bb;
...
bool Position::move_legal(const Move move) const{
    ...
    if(p == NO_PIECE || p % 2 != stm) return false;
        Bitboard bbFrom = Bitboard(1)<<move_from(move);
        Bitboard bbTo = Bitboard(1)<<move_to(move);
    Bitboard dest;
    ...
    if(p == WP){
        ...
        if(t == MoveQuiet){
            ...
            if(move_to(move) == current->ep && bbFrom
               & PawnAttack[White][current->ep])
                return true;
        }
        else if(t >= MovePN && t <= MovePQ){
            dest = (~occupied) & BB_RANK_1;
        ...
        if(t == MoveQuiet){
            if(king_attack(move_from(move)) & (~occupied) & bbTo
               & ~king_attack(first_bit(bitboard[KING+(stm^1)])))
                return true;
        }
        else if(t == MoveCapture){
            if(king_attack(move_from(move)) & bitboard[stm^1] & bbTo
               & ~king_attack(first_bit(bitboard[KING+(stm^1)])))
                return true;
        }
...
bool Position::move_gen_legal(const Move move) const{
    ...
    switch(move_type(move)){
        ...
        default: {
            std::cerr<<"Wrong move type (legal)!"<<std::endl;
            exit(1);
        }
    };
    return true;
...
bool Position::is_move_check(const Move move) const{
    ...
    if(p <= BP){
        if(PawnAttack[stm][to] & bitboard[KING + (stm^1)]) return true;
        const Square kSq = first_bit(bitboard[KING + (stm^1)]);
        if(discover_check(from,kSq)
            && Direction[kSq][from] != Direction[from][to])
            return true;
        Move type = move_type(move);
        if(type == MoveEP){
            Square cSq;
            ...
            const Bitboard bb = occupied
                                ^ (Bitboard(1) << from)
                                ^ (Bitboard(1) << to)
                                ^ (Bitboard(1) << cSq);
            if((bb_bishop_attack(kSq,bb) &
                (bitboard[QUEEN+stm] | bitboard[BISHOP+stm])) ||
                (bb_rook_attack(kSq,bb) &
                 (bitboard[QUEEN+stm] | bitboard[ROOK+stm])))
                return true;
        }
        else if(type >= MovePN && type <= MovePQ){
            p = Piece(type + stm);
            if((*bb_attack[p])(to,occupied
                               ^ (Bitboard(1)<<from))
                & bitboard[KING + (stm^1)])
                return true;
        }
        ...
        if((*bb_attack[p])(to,occupied ^ (Bitboard(1)<<from))
            & bitboard[KING + (stm^1)])
            return true;
        const Square kSq = first_bit(bitboard[KING + (stm^1)]);
        if(discover_check(from,kSq)
            && Direction[from][kSq] != Direction[from][to]) return true;
    }
    else{
        const Square kSq = first_bit(bitboard[KING + (stm^1)]);
        if(discover_check(from,kSq)
            && Direction[from][kSq] != Direction[from][to])
            return true;
        Move type = move_type(move);
        if(type == MoveCastle){
            const Bitboard bb = occupied ^ (Bitboard(1)<<from);
            Square rTo;
            switch(to){
                ...
                default:{
                    std::cerr<<"Wrong square (move_is_check)"<<std::endl;
                    exit(1);
                }
            };
            if(bb_rook_attack(kSq,bb) & (Bitboard(1) << rTo)) return true;
        }
    }
    return false;
}

// Incomplete (discovered check and king attack)
bool Position::is_move_attacks_king(const Move move) const{
    const Square from = move_from(move);
    const Square to = move_to(move);
    const Square kSq = first_bit(bitboard[KING + (stm^1)]);
    Piece p = piece[from];
    Bitboard bb_k_sq = king_attack(kSq);

    if(p <= BP){
        if((PawnAttack[stm][from] & bb_k_sq) == 0){
            if(PawnAttack[stm][to] & bb_k_sq) return true;
        }
    }
    else if(p <= BQ){
        if(((*bb_attack[p])(from,occupied) & bb_k_sq) == 0){
            if((*bb_attack[p])(to,occupied ^ (Bitboard(1)<<from)) & bb_k_sq)
                return true;
        }
    }
    return false;
...
int Position::move_value(const Move move) const{
    ...
    switch (type){
        ...
        case MovePN:        { return piece_value(Piece(KNIGHT))
                                        - piece_value(Piece(PAWN)) ; }
        case MovePB:        { return piece_value(Piece(BISHOP))
                                        - piece_value(Piece(PAWN)) ; }
        case MovePR:        { return piece_value(Piece(ROOK))
                                        - piece_value(Piece(PAWN)) ; }
        case MovePQ:        { return piece_value(Piece(QUEEN))
                                        - piece_value(Piece(PAWN)) ; }
        default:{
            std::cerr<<"Wrong move type (move_value)"<<std::endl;
            exit(1);
        }
    };
}
...
void Position::move_do(const std::string move){
    Thread thread;
    thread.pos = *this;
    MoveGenerator mg(thread,NullMove,1,1);
    Move m;
    int eval;
