...
inline void Position::add(const Piece p, const Square sq){
        const Bitboard bb = Bitboard(1) << sq;
...
bool Position::is_ok()const{
    ...
    for(Square sq=A1; sq<=H8; sq++){
        ...
        if(p == NO_PIECE){
            for(Piece cp=W; cp<=BK; cp++){
                                if((bitboard[cp] & (Bitboard(1)<<sq)) != 0){
                    std::cerr<<"Wrong bitboard(1)["<<piece_to_char(cp)<<"]";
                    std::cerr<<"sq="<<square_to_string(sq)<<std::endl;
                    return false;
                }
            }
                        if(((Bitboard(1) << sq) & occupied) != 0){
            ...
            mat_key ^= MatKey[p][count[p]];
                ...
                if((bitboard[p%2] & (Bitboard(1)<<sq)) == 0){
            std::cerr<<"Wrong bitboard["<<piece_to_char(Piece(p%2))<<"]";
            std::cerr<<"sq="<<square_to_string(sq)<<std::endl;
            return false;
        }
                if((bitboard[(p%2)^1] & (Bitboard(1)<<sq)) != 0){
            std::cerr<<"Wrong bitboard["<<piece_to_char(Piece((p%2)^1))<<"]";
            std::cerr<<"sq="<<square_to_string(sq)<<std::endl;
            return false;
        }
                if((bitboard[p] & (Bitboard(1)<<sq)) == 0){
            std::cerr<<"Wrong bitboard["<<piece_to_char(p)<<"]";
            std::cerr<<"sq="<<square_to_string(sq)<<std::endl;
            return false;
        }
                if(((Bitboard(1) << sq) & occupied) == 0){
        ...
        for(Piece j= WP; j<=BK; j++){
            if(j == p) continue;
                        if((bitboard[j] & (Bitboard(1)<<sq)) != 0){
                std::cerr<<"Wrong bitboard["<<piece_to_char(j)<<"]";
                std::cerr<<"sq="<<square_to_string(sq)<<std::endl;
                return false;
            }
        }
    ...
    if(king_attack(first_bit(bitboard[WK])) & bitboard[BK]){
        std::cerr<<"Wrong position (kings are neighbor)"<<std::endl;
        return false;
    }
    for(Piece p=Piece(KNIGHT); p<Piece(QUEEN); p++){
        if((bitboard[p] != 0 && bit_count(bitboard[p]) != num[p])
            || num[p] > 10){
...
bool Position::perpetual_check() const{
    for(int i=2; i<Search::ids_depth; i+=2){
        if(!ph[this->ply-i].checkers)
            return false;
    }
    return true;
}
...
void Position::gtb_info(unsigned int ws[5], unsigned int bs[5],
                        unsigned char wp[5], unsigned char bp[5]) const{

    int sq, piece, wc = 0, bc = 0;
    ...
    while (bb) {
        ...
        piece = this->piece[sq];
        switch(piece){
            case WP:{ws[wc] = sq; wp[wc++] = tb_PAWN; break;}
            case WN:{ws[wc] = sq; wp[wc++] = tb_KNIGHT; break;}
            case WB:{ws[wc] = sq; wp[wc++] = tb_BISHOP; break;}
            case WR:{ws[wc] = sq; wp[wc++] = tb_ROOK; break;}
            case WQ:{ws[wc] = sq; wp[wc++] = tb_QUEEN; break;}
            case WK:{ws[wc] = sq; wp[wc++] = tb_KING; break;}
            case BP:{bs[bc] = sq; bp[bc++] = tb_PAWN; break;}
            case BN:{bs[bc] = sq; bp[bc++] = tb_KNIGHT; break;}
            case BB:{bs[bc] = sq; bp[bc++] = tb_BISHOP; break;}
            case BR:{bs[bc] = sq; bp[bc++] = tb_ROOK; break;}
            case BQ:{bs[bc] = sq; bp[bc++] = tb_QUEEN; break;}
            case BK:{bs[bc] = sq; bp[bc++] = tb_KING; break;}
        }
    }
    ws[wc] = bs[bc] = tb_NOSQUARE;
    wp[wc] = bp[bc] = tb_NOPIECE;
}
...
std::string Position::move_to_san(const Move move) const{
    ...
    if (type == MoveCastle) {
        ...
        if (!is_pawn){
            san = SanPieceChar[piece[from]];

            Bitboard attack = 0;
            switch (piece[from]) {
                case WN: case BN:
                    { attack = bitboard[WN+stm] & knight_attack(to); break; }
                case WB: case BB:
                    { attack = bitboard[WB+stm] & bishop_attack(to); break; }
                case WR: case BR:
                    { attack = bitboard[WR+stm] & rook_attack(to); break; }
                case WQ: case BQ:
                    { attack = bitboard[WQ+stm] & queen_attack(to); break; }
                case WK: case BK:
                    { attack = bitboard[WK+stm] & king_attack(to); break; }
                default: {
                    std::cerr<<"Something really wrong (move to san)!"<<std::endl;
                    exit(1);
                }
            }

                        attack ^= Bitboard(1)<<from;
            attack &= bitboard[stm];
            if (attack) {
    ...
    if (thread.pos.is_in_check()){
        MoveGenerator mg(thread,NullMove,0,0);
        if(mg.next(thread,eval) == NullMove) san += '#';
        else san += '+';
    }
