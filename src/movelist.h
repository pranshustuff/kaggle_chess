...
inline void MoveList::add(const Move m, const int v){
    eval[size] = v; move[size++] = m;
}

inline void MoveList::add(const Move m){
    move[size++] = m;
}

inline Move MoveList::get(const int i) const{
    return move[i];
}
...
inline int MoveList::get_size() const {
    return size;
}
