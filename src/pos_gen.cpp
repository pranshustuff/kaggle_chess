        ...
        while(pawn_dest[i]){
            ...
            if(to >= A8 || to <= H1){
                for(Move m = MovePQ; m >= MovePN; m-=2)
                    ml.add(move_create(m,from,to),
                           piece_value(Piece(m))+
                           piece_value(piece[to])-
                           piece_value(WP));
            }
            else ml.add(move_create(MoveCapture,from,to),
                        piece_value(piece[to])-piece_value(WP));
        }
    }
    ...
    for(Piece p=KNIGHT+stm; p<=BQ; p+=2){
        ...
        while(bb){
            ...
            while(dest){
                ...
                ml.add(move_create(MoveCapture,from,to),
                       piece_value(piece[to])-piece_value(piece[from]));
            }
        }
    }

    from = first_bit(bitboard[WK+stm]);
    dest = king_attack(from) & bitboard[stm^1] &
           ~king_attack(first_bit(bitboard[WK+(stm^1)]));
    while(dest){
        to = first_bit_clear(dest);
        ml.add(move_create(MoveCapture,from,to),
               piece_value(piece[to])-piece_value(WK));
    }
}
...
void Position::gen_quiet(MoveList& ml) const{
        ...
        while(bb){
            ...
            while(dest){
                ...
                ml.add(move_create(MoveQuiet,from,to),
                       Search::history[p][from][to]/*+Search::his_diff[p][to]*/);
            }
        }
    }
    ...
    if(stm == White){
        ...
        if(!current->checkers && current->castle & W_CASTLE){
            if(current->castle & WK_CASTLE
                ...
                ml.add(move_create(MoveCastle,E1,G1),
                       Search::history[WK][E1][G1]/*+Search::his_diff[WK][G1]*/);
            if(current->castle & WQ_CASTLE
                && (((BB_B1|BB_C1|BB_D1)&empty) == (BB_B1|BB_C1|BB_D1))
                && !sq_attacked(D1,White)
                && ((BB_B2|BB_C2|BB_D2) & bitboard[BK]) == 0)
                ml.add(move_create(MoveCastle,E1,C1),
                       Search::history[WK][E1][C1]/*+Search::his_diff[WK][C1]*/);
        }
    }else{
        bb = bitboard[BP];
        ...
        if(!current->checkers && current->castle & B_CASTLE){
            if(current->castle & BK_CASTLE
                ...
                ml.add(move_create(MoveCastle,E8,G8),
                       Search::history[BK][E8][G8]/*+Search::his_diff[BK][G8]*/);
            if(current->castle & BQ_CASTLE
                && (((BB_B8|BB_C8|BB_D8)&empty) == (BB_B8|BB_C8|BB_D8))
                && !sq_attacked(D8,Black)
                && ((BB_B7|BB_C7|BB_D7) & bitboard[WK]) == 0)
                ml.add(move_create(MoveCastle,E8,C8),
                       Search::history[BK][E8][C8]/*+Search::his_diff[BK][C8]*/);
        }
    }
    ...
    for(int i=0; i<2; i++){
        while(pawn_dest[i]){
            ...
            from = to + PawnFrom[stm][i];
            ml.add(move_create(MoveQuiet,from,to),
                   Search::history[PAWN+stm][from][to]/*+Search::his_diff[PAWN+stm][to]*/);
        }
    }

    from = first_bit(bitboard[KING+stm]);
    dest = king_attack(from) & empty &
            ~king_attack(first_bit(bitboard[WK+(stm^1)]));
    while(dest){
        to = first_bit_clear(dest);
        ml.add(move_create(MoveQuiet,from,to),
               Search::history[KING+stm][from][to]/*+Search::his_diff[KING+stm][to]*/);
    }
}

void Position::gen_evasions(MoveList & ml) const{
    Square from, to, checkerSq, kSq;
    Piece p;
    Bitboard bb, dest, checkers, possible = 0, pawn_dest[4];

    kSq = first_bit(bitboard[KING+stm]);
    dest = king_attack(kSq) & ~bitboard[stm] &
            ~king_attack(first_bit(bitboard[KING+(stm^1)]));
    while(dest){
        to = first_bit_clear(dest);
                if(ml.get_size() >= 2 || !sq_attacked(to,stm,occupied^(Bitboard(1)<<kSq))){
            if(piece[to] != NO_PIECE)
                ml.add(move_create(MoveCapture,kSq,to),
                       HistoryMax + piece_value(piece[to])-piece_value(WK));
            else ml.add(move_create(MoveQuiet,kSq,to),
                        Search::history[KING+stm][kSq][to]);
        }
    }
    ...
    if((checkers & (checkers -1))  == 0){
        ...
        if(stm == White){
            bb = bitboard[WP];
            pawn_dest[0] = shift_up(shift_up(bb) & empty) & empty
                           & possible & BB_RANK_4;
            ...
            bb = bitboard[BP];
            pawn_dest[0] = shift_down(shift_down(bb) & empty) & empty
                           & possible & BB_RANK_5;
            ...
            while(pawn_dest[i]){
                ...
                if(ml.get_size() >= 2 || !is_pinned(from,to)){
                    if(to >= A8 || to <= H1){
                        for(Move m = MovePQ; m >= MovePN; m-=2)
                            ml.add(move_create(m,from,to),
                                   HistoryMax+piece_value(Piece(m))
                                   +piece_value(piece[to])-piece_value(WP));
                    }
                    else{
                        if(i<2)
                            ml.add(move_create(MoveQuiet,from,to),
                                   Search::history[KING+stm][from][to]);
                        else
                            ml.add(move_create(MoveCapture,from,to),
                                   HistoryMax+piece_value(piece[to])
                                   -piece_value(WP));
                    }
                }
            }
        ...
        if(current->ep != NO_SQ){
            if(stm == White){
                                if((Bitboard(1) << (current->ep-8)) & possible){
                    dest = bitboard[WP] & PawnAttack[Black][current->ep];
                    while(dest){
                        from = first_bit_clear(dest);
                        if(ml.get_size() >= 2
                           || !is_pinned(from,current->ep,current->ep-8))
                            ml.add(move_create(MoveEP,from,current->ep),
                                   HistoryMax);
                    }
                }
            }
                    ...
                    if(!ml.get_size() >= 2 || !is_pinned(from,to)){
                        if(piece[to] != NO_PIECE)
                            ml.add(move_create(MoveCapture,from,to),
                                   HistoryMax+piece_value(piece[to])
                                   -piece_value(p));
                        else ml.add(move_create(MoveQuiet,from,to),
                                    Search::history[p][from][to]);
                    }
                }
            }
        }
...
bool Position::discover_check(const Square sq, const Square kSq) const{
    Bitboard bb, attack;

    bb = bitboard[BISHOP+stm] | bitboard[QUEEN+stm];
    attack = bishop_attack(kSq) & bitboard[stm];
    if(attack & Bitboard(1)<<sq){
        if(bb_bishop_attack(kSq,occupied^(Bitboard(1)<<sq)) & bb)
            return true;
    }
    ...
    if(attack & Bitboard(1)<<sq){
        if(bb_rook_attack(kSq,occupied^(Bitboard(1)<<sq)) & bb)
            return true;
    }
    return false;
}
...
void Position::gen_quiet_checks(MoveList & ml) const{
    ...
    if(stm == White){
        ...
        if(f > FILE_A && f <FILE_H)
            attack1 = bb & ~disc & (BB_FILE[f-1] | BB_FILE[f+1]);
